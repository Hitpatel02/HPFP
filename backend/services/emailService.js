...
async function logEmailToDatabase(clientId, emailTo, emailSubject, emailBody, reminderNumber, documentMonth, status = 'sent') {
  try {
    // Use only the standardized field names
    await db.query(
      `INSERT INTO "user".email_logs 
          (client_id, email_to, email_subject, email_body, reminder_number, document_month, status) 
  ...
  await db.query(
    `UPDATE "user".client_documents 
     SET ${columnPrefix}${reminderNumber}_sent = TRUE, 
         ${columnPrefix}${reminderNumber}_sent_date = CURRENT_TIMESTAMP 
