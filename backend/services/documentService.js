...
async function createDocumentForClient(clientId, documentMonth = null) {
  try {
    ...
    if (existingDoc) {
      console.log(`Document for client ${client.name} (ID: ${clientId}) and month ${month} already exists with ID ${existingDoc.id}`);
      return existingDoc;
    }
    ...
    console.log(`Created document for client ${client.name} (ID: ${clientId}) for month ${month}`);
    return document;
  } catch (error) {
