 ...
 * @access  Private
 */
router.get('/whatsapp/download', authenticateToken, async (req, res) => {
  try {
    const { startDate, endDate } = req.query;
 ...
 * @access  Private
 */
router.get('/email/download', authenticateToken, async (req, res) => {
  try {
    const { startDate, endDate } = req.query;
 ...
 * @access  Private
 */
router.delete('/whatsapp', authenticateToken, async (req, res) => {
  try {
    const { startDate, endDate } = req.query;
    ...
    if (!tableExists) {
      return res.json({ message: 'No logs to clear' });
    }
    ...
    res.json({ 
      message: `Successfully cleared ${result.rowCount} WhatsApp logs`, 
...
router.delete('/email', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ 
      message: `Successfully cleared ${result.rowCount} email logs`, 
      count: result.rowCount 
    });
  } catch (error) {
...
/**
 * Helper function to format date time for CSV
 */
function formatDateTime(dateTimeString) {
  try {
    ...
    // Use MM/DD/YYYY format which Excel recognizes as a date by default
