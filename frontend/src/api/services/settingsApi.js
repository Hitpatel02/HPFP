...
const settingsAPI = {
  ...
  getSettings: async (token) => {
    return apiRequest('/api/settings/reminder', {
  ...
  getAvailableMonths: async (token) => {
    return apiRequest('/api/settings/reminder/months', {
      headers: getAuthHeaders(token)
    });
  },
  ...
  getSettingsForMonth: async (token, year, month) => {
    return apiRequest(`/api/settings/reminder/${year}/${month}`, {
      headers: getAuthHeaders(token)
    });
  },
  ...
  updateSettings: async (token, id, settings) => {
    ...
    return apiRequest(`/api/settings/reminder/${numericId}`, {
      method: 'PUT',
      headers: getAuthHeaders(token),
      body: JSON.stringify(settings)
    });
