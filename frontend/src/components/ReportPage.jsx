...
const ReportPage = () => {
  ...
  const fetchReport = async () => {
    try {
      ...
      const reportResult = await api.reports.generateReport(
        token, 
        dateRange.startDate, 
        dateRange.endDate
      );
      ...
      if (reportResult && reportResult.filePath) {
        const reportData = await api.reports.getReportData(
          token, 
          dateRange.startDate, 
          dateRange.endDate
        );
  ...
  const handleDownload = async () => {
    try {
      ...
      const blob = await api.reports.downloadReport(
        token, 
        dateRange.startDate, 
        dateRange.endDate
      );
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      const csvData = await api.reports.downloadReportCSV(
        token, 
        dateRange.startDate, 
        dateRange.endDate
      );
