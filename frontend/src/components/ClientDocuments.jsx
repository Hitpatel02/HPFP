...
const ClientDocuments = () => {
  ...
  const fetchClientAndDocuments = async () => {
    try {
      ...
      const clientData = await api.clients.getById(token, clientId);
      setClient(clientData);
      
      // Fetch client documents using API service
      const documentsData = await api.documents.getByClient(token, clientId);
  ...
  const handleAddDocument = async (e) => {
    ...
    try {
      ...
      await api.documents.create(token, clientId, {
        ...newDocument,
        gst_1_received_date: newDocument.gst_1_received_date || null,
        bank_statement_received_date: newDocument.bank_statement_received_date || null,
        tds_received_date: newDocument.tds_received_date || null
      });
  ...
  const handleUpdateDocument = async (id, updates) => {
    try {
      ...
      await api.documents.update(token, id, processedUpdates);
  ...
  const handleDeleteDocument = async () => {
    ...
    try {
      setIsDeleting(true);
      await api.documents.delete(token, documentToDelete);
  ...
  const handleCreateCurrentMonthDocument = async () => {
    try {
      setIsGeneratingCurrentMonthDoc(true);
      setError(null);
      setSuccessMessage(null);
      
      const result = await api.documents.createForClient(token, clientId);
      
      if (result.isNewDocument) {
        setSuccessMessage(`Created new document for ${client.name} for ${result.currentMonth}`);
      } else {
        setSuccessMessage(`Document for ${result.currentMonth} already exists`);
      }
      ...
      setIsGeneratingCurrentMonthDoc(false);
    }
  };
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
          <Button 
            ...
            disabled={isGeneratingCurrentMonthDoc}
            className="me-2"
          >
            {isGeneratingCurrentMonthDoc ? (
              <>
