...
const ReminderSettings = () => {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (response && Object.keys(response).length > 0) {
        ...
        const dateFields = [
          'today_date', 'gst_due_date', 'gst_reminder_1_date', 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add month and year as separate fields for easier querying
      formDataToSubmit.month = selectedMonth + 1; // 1-based month
      formDataToSubmit.year = selectedYear;
