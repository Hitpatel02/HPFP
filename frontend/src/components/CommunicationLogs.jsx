...
const CommunicationLogs = () => {
  ...
  const fetchWhatsappLogs = async () => {
    try {
      ...
      const logs = await api.logs.getWhatsappLogs(token, dateRange.startDate, dateRange.endDate);
  ...
  const fetchEmailLogs = async () => {
    try {
      ...
      const logs = await api.logs.getEmailLogs(token, dateRange.startDate, dateRange.endDate);
  ...
  const downloadWhatsappLogs = async () => {
    try {
      ...
      if (!logs || logs.length === 0) {
        try {
          logs = await api.logs.getWhatsappLogs(token, dateRange.startDate, dateRange.endDate);
  ...
  const downloadEmailLogs = async () => {
    try {
      ...
      if (!logs || logs.length === 0) {
        try {
          logs = await api.logs.getEmailLogs(token, dateRange.startDate, dateRange.endDate);
  ...
  const clearWhatsappLogs = async () => {
    try {
      ...
      await api.logs.clearWhatsappLogs(token, dateRange.startDate, dateRange.endDate);
  ...
  const clearEmailLogs = async () => {
    try {
      ...
      await api.logs.clearEmailLogs(token, dateRange.startDate, dateRange.endDate);
