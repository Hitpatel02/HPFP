...
const ClientList = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      const data = await api.clients.getAll(token);
  ...
  const handleAddClient = async (e) => {
    ...
    try {
      setIsCreating(true);
      await api.clients.create(token, newClient);
  ...
  const handleDeleteClient = async () => {
    ...
    try {
      setIsDeleting(true);
      await api.clients.delete(token, clientToDelete.id);
  ...
  const handleCreateDocuments = async () => {
    try {
      ...
      setError(null);
      const result = await api.documents.createForAll(token);
      ...
      if (result.count === 0 && result.existingCount === result.totalClients) {
        ...
        // When all documents were newly created
        setSuccessMessage(`Successfully created ${result.count} documents for ${result.currentMonth} for all ${result.totalClients} clients`);
        
        // Refresh the client list to ensure we have the latest data
        fetchClients();
      } else {
        // Fallback message
        setSuccessMessage(`Operation completed. ${result.count} documents created, ${result.existingCount} already existed (total clients: ${result.totalClients})`);
      }
      
      // Clear success message after 8 seconds (increased from 5 for better readability)
      setTimeout(() => {
        setSuccessMessage(null);
      }, 8000);
    } catch (err) {
  ...
  const handleCleanupDuplicates = async () => {
    try {
      ...
      const result = await api.documents.cleanupDuplicates(token);
      
      if (result.duplicatesFound > 0) {
        setSuccessMessage(`Found and fixed ${result.duplicatesFound} clients with duplicate documents. Removed ${result.recordsRemoved} duplicate records.`);
      } else {
        setSuccessMessage('No duplicate documents found. Your database is clean!');
      }
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
          <Button 
            ...
            className="me-2"
            title="Fix database inconsistencies by removing duplicate document entries"
          >
