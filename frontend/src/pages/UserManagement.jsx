...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const data = await api.auth.getUsers(token);
      setUsers(data);
      setError(null);
    } catch (err) {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      setIsDeleting(true);
      await api.auth.deleteUser(token, userToDelete.id);
  ...
  const handleAddUser = async (e) => {
    ...
    try {
      ...
      await api.auth.createUser(token, userData);
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      await api.auth.updateUser(token, userToEdit.id, userData);
